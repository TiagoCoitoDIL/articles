...
In this session we talked about preserving and maximizing the value of digital media assets using Amazon S3 by talking about three biggest trends shaping on how customers in media are using S3 to preserve and maximize the value of their media.
...
If we think for minute most of the data is cold.. meaning we don't use it so often. For those kind of data we should use [Amazon S3 Glacier](https://aws.amazon.com/s3/storage-classes/glacier/?nc1=h_ls).
...
Second trend, **Preserving your most princeless assets**.
There are some data that is critical for enterprises and S3 can be a safe place to store our data.
...
Finalizing the session by mention if we want to learn more we can check for more content [here](https://aws.amazon.com/pt/training/learn-about/storage/).
